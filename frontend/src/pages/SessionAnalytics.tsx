...
export default function SessionAnalytics() {
    ...
    return (
      <div className="p-6 flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4">
          ...
          <button onClick={() => navigate('/dashboard')} className="text-indigo-400 hover:text-indigo-300 text-sm transition">
  ...
  const tooltipStyle = {
    contentStyle: { backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.3)' },
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={downloadCsv}
          className="px-4 py-2 bg-slate-800/80 hover:bg-slate-700 border border-slate-700/50 rounded-xl text-sm text-slate-300 transition flex items-center gap-2"
        >
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        {/* Title card */}
        <div className="bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border border-indigo-500/20 rounded-2xl p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-xl md:text-2xl font-bold text-white">{session.quiz_title}</h1>
              </div>
              <div className="flex items-center gap-4 text-sm text-slate-400">
                <span className="font-mono bg-slate-800/50 px-2 py-0.5 rounded">{session.code}</span>
            ...
            <div className="flex items-center gap-2">
              <div className={`text-center px-4 py-2 rounded-xl ${global_stats.success_rate >= 50 ? 'bg-emerald-500/15 border border-emerald-500/25' : 'bg-red-500/15 border border-red-500/25'}`}>
                <p className={`text-2xl font-bold ${global_stats.success_rate >= 50 ? 'text-emerald-400' : 'text-red-400'}`}>{global_stats.success_rate}%</p>
                <p className="text-xs text-slate-500">reussite</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'overview' && (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Bar chart */}
              <section className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 md:p-6">
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={barData} margin={{ top: 5, right: 10, bottom: 5, left: -10 }}>
                    ...
                    <Legend wrapperStyle={{ color: '#94a3b8', fontSize: 12, paddingTop: '12px' }} />
                    <Bar dataKey="correct" name="Correctes" stackId="a" fill="#22c55e" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="incorrect" name="Incorrectes" stackId="a" fill="#ef4444" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </section>
              ...
              <section className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 md:p-6">
                  ...
                  <div className="flex items-end justify-center gap-2 mb-5 px-2">
                    {leaderboard.length >= 2 && (
                      <div className="flex flex-col items-center flex-1">
                        ...
                        <p className="text-sm font-semibold text-slate-200 truncate max-w-full">{leaderboard[1].nickname}</p>
                        <p className="text-xs font-mono text-indigo-400">{leaderboard[1].score} pts</p>
                        <div className="w-full h-16 bg-gradient-to-t from-slate-600 to-slate-500 rounded-t-lg mt-2 flex items-center justify-center">
                    ...
                    <div className="flex flex-col items-center flex-1">
                      ...
                      <p className="text-sm font-bold text-yellow-300 truncate max-w-full">{leaderboard[0].nickname}</p>
                      <p className="text-xs font-mono text-yellow-400">{leaderboard[0].score} pts</p>
                      <div className="w-full h-24 bg-gradient-to-t from-yellow-600 to-yellow-400 rounded-t-lg mt-2 flex items-center justify-center">
                    ...
                    {leaderboard.length >= 3 && (
                      <div className="flex flex-col items-center flex-1">
                        <span className="text-2xl mb-1">&#x1F949;</span>
                        <p className="text-sm font-semibold text-slate-200 truncate max-w-full">{leaderboard[2].nickname}</p>
                        <p className="text-xs font-mono text-indigo-400">{leaderboard[2].score} pts</p>
                        <div className="w-full h-12 bg-gradient-to-t from-amber-700 to-amber-500 rounded-t-lg mt-2 flex items-center justify-center">
                ...
                <div className="space-y-1.5 max-h-64 overflow-y-auto">
                  {leaderboard.slice(3).map((p) => (
                    <div key={p.rank} className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-700/30 hover:bg-slate-700/50 transition">
                      <div className="flex items-center gap-2.5">
                        <span className="w-6 h-6 rounded-full bg-slate-700 text-slate-400 flex items-center justify-center text-xs font-bold">{p.rank}</span>
                        <span className="text-sm font-medium text-slate-200">{p.nickname}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-slate-500 hidden sm:inline">{p.correct_answers}/{p.total_answers}</span>
                        <span className="text-sm font-mono text-indigo-400">{p.score} pts</span>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-700/50">
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {leaderboard.map((p) => (
                            <tr key={p.rank} className="border-t border-slate-700/30 text-slate-300">
                              <td className="py-1.5 font-medium">{p.rank}</td>
                              <td className="py-1.5">{p.nickname}</td>
                              <td className="py-1.5 text-right font-mono text-indigo-400">{p.score}</td>
                  ...
                  <div key={q.question_id} className="group">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-500 shrink-0">
                        ...
                        <span className={`font-semibold ${q.correct_percentage >= 50 ? 'text-emerald-400' : 'text-red-400'}`}>
                    ...
                    <div className="h-3 bg-slate-700/50 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${
                          q.correct_percentage >= 70 ? 'bg-gradient-to-r from-emerald-500 to-green-400' :
                          q.correct_percentage >= 40 ? 'bg-gradient-to-r from-yellow-500 to-amber-400' :
        ...
        {activeTab === 'questions' && (
          <div className="space-y-5">
            {questions.map((q, i) => (
              <div key={q.question_id} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden">
                {/* Question header */}
                <div className="px-5 md:px-6 pt-5 md:pt-6 pb-4">
                  <div className="flex items-start justify-between gap-4 mb-1">
                    <h3 className="text-base font-semibold text-white">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-indigo-500/20 text-indigo-400 text-sm font-bold mr-2.5">{i + 1}</span>
                      {q.text}
                    </h3>
                  </div>
                  <div className="flex items-center gap-4 mt-3 ml-9">
                    <div className={`flex items-center gap-1.5 text-sm font-medium ${q.correct_percentage >= 50 ? 'text-emerald-400' : 'text-red-400'}`}>
                    ...
                    <span className="text-sm text-slate-500">{q.total_responses} reponse{q.total_responses > 1 ? 's' : ''}</span>
                    <span className="text-sm text-slate-500">{q.avg_response_time}s en moy.</span>
                  </div>
                </div>
                ...
                {q.image_url && (
                  <div className="px-5 md:px-6 pb-4">
                    <img
                      src={q.image_url}
                      alt="Question"
                      className="max-h-48 object-contain rounded-xl bg-slate-900/50 cursor-pointer hover:opacity-80 transition"
                    ...
                    <div key={a.answer_id} className="group">
                      <div className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 ${
                          a.is_correct ? 'bg-emerald-500/20 border border-emerald-500/40' : 'bg-slate-700/50 border border-slate-600/30'
                        }`}>
                          {a.is_correct && (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <span className={`text-sm truncate ${a.is_correct ? 'text-emerald-300 font-medium' : 'text-slate-300'}`}>{a.text}</span>
                            <span className="text-xs text-slate-500 ml-2 shrink-0">{a.count} ({a.percentage}%)</span>
                          </div>
                          <div className="h-2 bg-slate-700/50 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all duration-500 ${a.is_correct ? 'bg-emerald-500' : 'bg-slate-500'}`}
...
function KpiCard({ icon, label, value, sub, color = 'text-indigo-400' }: {
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
      ...
      {sub && <p className="text-slate-600 text-xs mt-1 truncate">{sub}</p>}
    </div>
  );
}
